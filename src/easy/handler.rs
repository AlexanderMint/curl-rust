...
use curl_sys;
use libc::{self, c_void, c_char, c_long, size_t, c_int, c_double};

use Error;
use easy::{List, Form};
use easy::{WriteError, ReadError, SeekResult, InfoType};
use panic;
...
impl<H: Handler> Easy2<H> {
    ...
    fn default_configure(&mut self) {
        ...
        // let cb: curl_sys::curl_ssl_ctx_callback = ssl_ctx_cb;
...
extern fn write_cb<H: Handler>(ptr: *mut c_char,
    ...
    panic::catch(|| unsafe {
        let input = slice::from_raw_parts(ptr as *const u8,
                                          size * nmemb);
        match (*(data as *mut H)).write(input) {
            Ok(s) => s,
...
extern fn read_cb<H: Handler>(ptr: *mut c_char,
    ...
    panic::catch(|| unsafe {
        let input = slice::from_raw_parts_mut(ptr as *mut u8,
                                              size * nmemb);
        match (*(data as *mut H)).read(input) {
            Ok(s) => s,
            Err(ReadError::Pause) => {
...
extern fn seek_cb<H: Handler>(data: *mut c_void,
    ...
    panic::catch(|| unsafe {
        ...
        (*(data as *mut H)).seek(from) as c_int
    }).unwrap_or(!0)
}
...
extern fn progress_cb<H: Handler>(data: *mut c_void,
    ...
    let keep_going = panic::catch(|| unsafe {
        (*(data as *mut H)).progress(dltotal, dlnow, ultotal, ulnow)
    }).unwrap_or(false);
    if keep_going {
        0
    } else {
...
extern fn debug_cb<H: Handler>(_handle: *mut curl_sys::CURL,
    ...
    panic::catch(|| unsafe {
        ...
        (*(userptr as *mut H)).debug(kind, data)
    });
    return 0
}
